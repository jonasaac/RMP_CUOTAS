  ...
  <div class="col-md-12">
              ...
              <select class="form-control input-xs" name="folio_detalles[_ids]" id="folio-detalles-ids" data-placeholder="Seleccione un el Detalle del Folio Indicado" lang="es" style="width: 100%">
                  ...
                  <?php foreach($folioDetalles as $detalle): ?>
                      <option value="<?=$detalle->ids?>"
                              ...
                              <?=explode(',',$detalle->ids)==$loteEncabezado->folio_detalles_ids?'selected':''?>>
    ...
    <div class="col-md-3">
      <div class="form-group">
        ...
        <div class="col-sm-9">
          <input type="text" class="form-control input-xs text-right" name="lote" id="nro-lote" readonly value="<?=$loteEncabezado->lote?>">
        </div>
      </div>
    </div>
    </div>
    ...
    <div class="row">
      <div class="col-sm-12">
        <table class="table table-hover table-condensed" id="detalles-table">
          ...
          <tbody>
            <?php $detalleIdx = 0; ?>
            <?php foreach ($loteEncabezado->lote_detalles as $detalle): ?>
              <tr>
                <td>
                  <?= $this->Form->input('lote_detalles.'.$detalleIdx.'.id', ['type' => 'hidden'])?>
                  <?= $this->Form->input('lote_detalles.'.$detalleIdx.'.pallet', ['placeholder' => 'Ingrese el nro. del Pallet']) ?>
                </td>
                <td>
                  <?= $this->Form->input('lote_detalles.'.$detalleIdx.'.calibre_id', ['placeholder' => 'Seleccione un Calibre', 'lang' => 'es', 'options' => $calibres, 'style' => 'width: 100%']) ?>
                </td>
                <td>
                  <div class="input-container">
                  <input type="text" name="lote_detalles[<?=$detalleIdx?>][unidades][0][_joinData][cantidad]"
                  class="form-control input-xs input-active has-success"
                  placeholder="Ingrese el número de Cajas"
                  required="required"
                  data-precision="0"
                  data-unidad-id="4"
                  value="<?=$this->Number->precision($detalle->unidades[0]->_joinData->cantidad, 0)?>">
                  <?= $this->Form->input('lote_detalles.'.$detalleIdx.'.unidades.0.id', ['type' => 'hidden', 'value' => '4']) ?>
                    <label>CJS</label>
                  </div>
                </td>
                <td>
                  <div class="input-container">
                    <input type="text" name="lote_detalles[<?=$detalleIdx?>][unidades][1][_joinData][cantidad]"
                    class="form-control input-xs input-active has-success"
                    placeholder="Ingrese la cantidad de kilos"
                    required="required"
                    data-precision="3"
                    data-unidad-id="3"
                    value="<?=$this->Number->precision($detalle->unidades[1]->_joinData->cantidad)?>">
                    <?= $this->Form->input('lote_detalles.'.$detalleIdx.'.unidades.1.id', ['type' => 'hidden', 'value' => '3']) ?>
                    <label>KLS</label>
                  </div>
                </td>
                <td class="centered">
...
<script>
 $(document).ready(function () {
     ...
     $('#sub-codigo, #folio-detalles-ids', $thisModal).prop("disabled", true);


     updateNroLote(); // Se actualiza con la información que se carga en un comienzo

     $('#folio-detalle-id, #sub-codigo', $thisModal).on('change', function() {
       // TODO(dcampos00): Se debe actualiza el nro de lote
