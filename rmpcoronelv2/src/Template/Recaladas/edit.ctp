...
?>
    <div class="row" id="<?=$hash_id?>">
        <div class="col-md-12">
          <form class="form-horizontal" id="recalada-form">
            <?= $this->Form->input('marea_id', ['type' => 'hidden'])?>
            <legend>Recalada - <?=$recalada->marea->nave->nombre?> - <?=ConvertirFecha($recalada->fecha_recalada->toUnixString())?></legend>
                ...
                <div class="col-sm-9">
                        <div class="input-group">
                          <select class="form-control input-xs" style="width: 100%" name="ponton_id" id="ponton-id" data-placeholder="Seleccione un pónton de recalada" placeholder="Seleccione un pónton de recalada" lang="es">
                            <option value></option>
                            <option value="<?=$recalada->ponton_id?>" selected="selected"><?=$recalada->ponton->nombre?></option>
                        </select>
            ...
            <div class="form-group">
              ...
              <div class="col-sm-9">
                <textarea id="observaciones" name="observaciones" class="form-control"><?=$recalada->observaciones?></textarea>
              </div>
            </div>
          </form>
        </div>
    ...
    <script>
     $(document).ready(function () {
         ...
         $("#ponton-id", $thisModal).select2();
         $('#fecha-recalada-date', $thisModal).val( moment('<?=$recalada->fecha_recalada->format(DateTime::ISO8601)?>').format('DD-MMM-YYYY') )
         $('#fecha-recalada-date-container', $thisModal).datetimepicker(dateOptions());
         $('#fecha-recalada-date-container', $thisModal).data('DateTimePicker').minDate( moment.utc(fechaMarea).set({'hour':0, 'minute':0}) );
         $('#fecha-recalada-time', $thisModal).val( moment('<?=$recalada->fecha_recalada->format(DateTime::ISO8601)?>').format('HH:mm') );
         $('#fecha-recalada-time-container', $thisModal).datetimepicker(timeOptions());
